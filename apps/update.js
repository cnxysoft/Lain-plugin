...
export class update extends plugin {
    ...
    async gitErr(err, stdout) {
        ...
        if (errMsg.includes('be overwritten by merge')) {
            await this.reply(msg + `存在冲突：\n${errMsg}\n` + '请解决冲突后再更新，或者执行#Lain/铃音强制更新，放弃本地修改')
            return
        }

        if (stdout.includes('CONFLICT')) {
            await this.reply([msg + '存在冲突咩\n', errMsg, stdout, '\n请解决冲突后再更新，或者执行#Lain/铃音强制更新，放弃本地修改'])
            return
        }
    ...
    async getLog(plugin = '') {
        ...
        return common.makeForwardMsg(this.e, [log, end], `${plugin || 'Lain-plugin'} 更新日志，共${line}条`)
    }

    async updateLog() {
        let log = await this.getLog('Lain-plugin')
        await this.reply(log)
    }
